...
def api_reload_startcards():
    """v0.4.1: Stub – wir speichern nur die zusammengesetzte URL."""
    st = load_state()
    base_url = st["settings"].get("startcards_base_url", "").strip()
    suffix = st["settings"].get("startcards_suffix", "").strip()

    full_url = ""
    if base_url:
        if base_url.startswith("http://") or base_url.startswith("https://"):
            full_url = base_url.rstrip("/") + "/" + suffix.lstrip("/")
        else:
            full_url = "http://" + base_url.rstrip("/") + "/" + suffix.lstrip("/")

    sc = st.get("startcards", {})
    sc["loaded"] = bool(base_url)
    sc["last_fetch_ts"] = now_ms()
    sc["row_count"] = 0
    sc["rows"] = []
    sc["last_error"] = None if base_url else "Keine Base-URL gesetzt"
    sc["url"] = full_url or None
    st["startcards"] = sc
    save_state(st)
    return jsonify({"ok": True, "startcards": sc})
