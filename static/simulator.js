...
(function(){
  ...
  async function sendTrigger(taster){
    const payload = { taster, ts: Date.now(), stopwatch_ms: (isRunning ? (Date.now() - startTs) : lastStopMs), run_id: `lauf-${currentLauf}` };
    try {
  ...
  function renderTopBar(el, leftText, rightText, accent){
    ...
    if (!top){ top = document.createElement('div'); top.className = 'topbar'; el.appendChild(top); }
  ...
  function renderMaindisplay(el, mainTime, name, disciplineText){
    ...
    if (!md){ md = document.createElement('div'); md.className = 'maindisplay'; el.appendChild(md); }
  ...
  function renderSeparator(el){
    ...
    if (!s){ s = document.createElement('div'); s.className = 'separator'; el.appendChild(s); }
  }

  function renderLastBlock(el, label, lasttime, meta){
    let lb = el.querySelector('.lastblock');
    if (!lb){ lb = document.createElement('div'); lb.className = 'lastblock'; el.appendChild(lb); }
  ...
  function renderBottomBar(el, humidity, temp, battery, letter, rssi){
    ...
    if (!bot){ bot = document.createElement('div'); bot.className = 'bottombar'; el.appendChild(bot); }
    // Wifi bars
